...
class NartisRfMeterComponent : public esphome::PollingComponent {
 public:
  ...
  /// The meter transmits/listens on a fixed bank (channel 0) and only encodes
 ...
 protected:
   ...
   * ~650 kHz BETWEEN sessions, landing 97..278 kHz off the default RX presets.
  ...
  /// Build 29-byte beacon payload matching the meter's beacon address builder.
  ...
  // (we sent seq=06 where the meter expects seq=01) and stay in a SESSION_SETUP
  // retransmit loop (the same SESSION_SETUP frame sent 3 times by the meter,
  // because our MODE-6 and BEACON were both seq-wrong).
  uint8_t data_seq_{2};
