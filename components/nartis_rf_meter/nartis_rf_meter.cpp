...
// Convert a CP1251 (Windows-1251) string to UTF-8. The meter encodes textual
// registers (name, model) in CP1251, which Home Assistant would otherwise show
// as mojibake. Bytes < 0x80 are ASCII and pass through unchanged; each high byte
// maps via `table` to its UTF-8 sequence, packed little-endian (1-3 bytes; a 0
// entry drops the byte). `out` must hold up to 3*strlen(in)+1 bytes.
static void cp1251_to_utf8_(char *out, const char *in) {
