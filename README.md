[СПОДЭС/DLMS/COSEM](https://github.com/latonita/esphome-dlms-cosem) •
[МЭК-61107/IEC-61107](https://github.com/latonita/esphome-iec61107-meter) •
[Энергомера МЭК/IEC](https://github.com/latonita/esphome-energomera-iec) •
[Энергомера CE](https://github.com/latonita/esphome-energomera-ce) •
[СПб ЗИП ЦЭ2727А](https://github.com/latonita/esphome-ce2727a-meter) •
[Ленэлектро ЛЕ-2](https://github.com/latonita/esphome-le2-meter) •
[Пульсар-М](https://github.com/latonita/esphome-pulsar-m) •
[Энергомера BLE](https://github.com/latonita/esphome-energomera-ble) •
[Нартис RF433](https://github.com/latonita/esphome-nartis-rf-meter) •
[Нартис RF433-2](https://github.com/latonita/esphome-nartis-rf-2-meter) •
[Нартис UART RF433-2](https://github.com/latonita/esphome-uart-nartis-rf) •
[Nordic UART (BLE NUS)](https://github.com/latonita/esphome-nordic-uart-ble) •

---

# esphome-nartis-rf-meter

Компонент ESPHome для связи со счётчиками электроэнергии **Нартис И100 / И300** по радиоканалу **433 МГц** с помощью ESP32 и радиомодуля **CMT2300A**.

Компонент заменяет собой выносной цифровой дисплей **Нартис Д101**: он **сопрягается** со счётчиком и далее **регулярно опрашивает** счётчик по протоколу **DLMS/COSEM**, забирая нужные показания по их OBIS-кодам. Полный список кодов можно найти в руководстве по эксплуатации дисплея Нартис Д101, а также приборов учета электроэнергии Нартис И100 и Нартис И300.

**ВАЖНО**: ПУ с дисплеем **Нартис Д101-2**, а также работающие по протоколу **RF-433-2** (большинство с 2024 г.в.) работатают с другими компонентами:
[Нартис RF433-2](https://github.com/latonita/esphome-nartis-rf-2-meter) или [Нартис UART RF433-2](https://github.com/latonita/esphome-uart-nartis-rf).

# Функции

## Реализованы

- **Активный опрос** счётчика по интервалу (`update_interval`).
- **Сопряжение (pairing)** со счётчиком: эмуляция probe -> ACK -> SESSION_SETUP -> подтверждение, как делает настоящий CIU.
- **Шифрование AES-128-GCM**; сессионный ключ автоматически извлекается из ответа счётчика при сопряжении (на основе серийного номера счётчика).
- **Автоматическое восстановление**: при сбое цикла компонент возвращается в ожидание и повторяет попытку на следующем интервале; после нескольких неудачных чтений подряд - автоматически выполняет повторное сопряжение.
- **Чтение пакетами** (`batch_size`) через DLMS get-request-with-list - несколько OBIS за один запрос.
- **Числовые и текстовые сенсоры** по произвольным OBIS-кодам и классам COSEM.
- **Авто-выбор канала** по RSSI или фиксация канала вручную.
- **Сохранение сессии в NVS**: ключ, счётчики кадров, адрес счётчика сохраняются во flash и восстанавливаются после перезагрузки - сопряжение заново не выполняется.


## Ограничения 

- Автоматическое масштабирование значений не происходит, т.к. счетчик не передает данные scaler/unit. Используйте `filter: - multiply: xxx`
- Опрос счетчика занимает в среднем около 30 секунд - не рекомендуется делать опрос чаще, чем раз 5 минут, дабы не засорять эфир

# Аппаратное обеспечение

Понадобится плата на **ESP32** и радиомодуль **CMT2300A** с антенной 433Мгц. Например, **XL2300-SMT**.
Работа с **ESP8266** не проверялась, но компиляция проходит.

Важно: CMT2300A использует нестандартный **3-проводной SPI**, поэтому обмен реализован программным («bit-bang») SPI через обычные GPIO, а не через аппаратный SPI-контроллер. Линия прерывания **GPIO3 (INT2) обязательна** - по ней отслеживается заполнение FIFO при приёме.

# Планы на будущее

Добавить поддержку других модулей, например **CC1101** - Нужны добровольцы со стендами для тестирования и, желательно, RTL-SDR

## Подключение CMT2300A к ESP32

| CMT2300A         | ESP32   | Описание                          |
|------------------|---------|-----------------------------------|
| SDIO             | GPIO 23 | Данные SPI (двунаправленная)      |
| SCLK             | GPIO 18 | Тактовый сигнал SPI               |
| CSB              | GPIO 27 | Chip Select (банк регистров)      |
| FCSB             | GPIO 26 | FIFO Chip Select                  |
| GPIO3 (INT2)     | GPIO 14 | Прерывание FIFO (обязательно)     |
| VDD              | 3.3V    | Питание                           |
| GND              | GND     | Общий провод                      |

> GPIO выбраны как пример - подставьте пины своей платы. Линию прерывания GPIO3 можно завести на любой вход; на ESP32 для этого подходят и пины 34-39 (только вход).

# Установка

```yaml
external_components:
  - source: github://latonita/esphome-nartis-rf-meter
    components: [nartis_rf_meter]
    refresh: 1s
```

# Быстрый старт

```yaml
external_components:
  - source: github://latonita/esphome-nartis-rf-meter
    components: [nartis_rf_meter]
    refresh: 1s

nartis_rf_meter:
  pin_sdio: GPIO23
  pin_sclk: GPIO18
  pin_csb: GPIO27
  pin_fcsb: GPIO26
  pin_gpio3: GPIO14

  meter_serial: "012345678901"   # 12 цифр с шильдика счётчика
  
  update_interval: 5min

sensor:
  - platform: nartis_rf_meter
    name: "Active Power"   # Активная мощность
    obis_code: "1.0.1.7.0.255"
    unit_of_measurement: W
    accuracy_decimals: 1
    device_class: power
    state_class: measurement
```

# Как это работает

1. По интервалу `update_interval` запускается цикл опроса.
2. Если сопряжение ещё не выполнено - компонент сканирует каналы по RSSI, выбирает канал и проходит handshake со счётчиком, получая сессионный ключ.
3. Если сопряжение уже есть - ключ и счётчики кадров переиспользуются, и сразу запускается чтение данных.
4. Запрашиваются настроенные сенсоры (по одному или пакетами `batch_size`), значения публикуются в Home Assistant.
5. По завершении цикла радио засыпает до следующего интервала.

> **О протоколе.** Это проприетарный протокол, построенный на идеях беспроводного M-Bus (wM-Bus, EN 13757): структура кадра (поля длины/контроля, адрес «производитель+серийник+версия+тип», CI-поле), коды функций control-поля и CRC-полином `0x3D65` заимствованы из wM-Bus, поверх - прикладной уровень DLMS/COSEM. Радиоуровень (433 МГц на CMT2300A), шифрование (AES-128-GCM) и процедура сопряжения - собственные, поэтому стандартными wM-Bus-стэками счётчик не читается.

# Конфигурация хаба (`nartis_rf_meter`)

| Параметр                      | Тип / по умолчанию | Описание                                                                                       |
|-------------------------------|--------------------|------------------------------------------------------------------------------------------------|
| `pin_sdio`                    | GPIO, обязательно  | Линия данных SPI.                                                                              |
| `pin_sclk`                    | GPIO, обязательно  | Тактовый сигнал SPI.                                                                           |
| `pin_csb`                     | GPIO, обязательно  | Chip Select банка регистров.                                                                   |
| `pin_fcsb`                    | GPIO, обязательно  | FIFO Chip Select.                                                                              |
| `pin_gpio3`                   | GPIO, обязательно  | Линия прерывания INT2 (заполнение FIFO при приёме).                                            |
| `meter_serial`                | строка, обязательно| 12-значный серийный номер счётчика с шильдика (например `"021245003137"`).                     |
| `update_interval`             | время, `5min`      | Период опроса счётчика.                                                                        |
| `fix_channel`                 | 0...3, опц.          | Зафиксировать радиоканал и отключить авто-выбор по RSSI.                                       |
| `use_alternative_channels`    | bool, `false`      | Использовать альтернативную сетку частот - иногда помогает, если счётчик не отвечает.          |
| `batch_size`                  | 1...10, `5`          | Сколько OBIS-атрибутов читать одним запросом get-request-with-list.                            |
| `rx_timeout`                  | время, `3000ms`    | Общий таймаут ожидания ответа (сопряжение и завершение приёма кадра).                          |
| `rx_reply_timeout`            | время, `900ms`     | Короткий таймаут ожидания ответа на GET - до прихода первого байта.                            |


# Сенсоры

### Числовой сенсор (sensor)

```yaml
sensor:
  - platform: nartis_rf_meter
    name: "Voltage L1"   # Напряжение фазы 1
    obis_code: "1.0.32.7.0.255"
    unit_of_measurement: V
    accuracy_decimals: 1
    device_class: voltage
    state_class: measurement
    filters:
      - multiply: 0.1   # при необходимости масштабируйте значение фильтром
```

Параметры: `obis_code` (обязательно), `obis_class` (по умолчанию `3`), `obis_attr` (по умолчанию `2`). Поддерживаются форматы OBIS `1.0.32.7.0.255`, `1-0:32.7.0.255`, `1-0:32.7.0*255`.

### Текстовый сенсор (text_sensor)

```yaml
text_sensor:
  - platform: nartis_rf_meter
    name: "Serial Number"   # Серийный номер
    obis_code: "0.0.96.1.0.255"
```

Octet-string данные декодируются как текст (байты - это коды символов: `30 32 33 ...` -> `"023..."`). Завершающие нулевые байты отбрасываются, управляющие байты (< 0x20) заменяются на `.`. Текстовые значения счётчика приходят в кодировке **CP1251** и автоматически конвертируются в **UTF-8**, поэтому кириллица (название, модель, производитель) отображается корректно.

Для объекта **Clock (класс 8)** укажите `obis_class: 8` - значение времени будет отформатировано как `ГГГГ-ММ-ДД ЧЧ:ММ:СС[.сс][ +/-ЧЧ:ММ]`:

```yaml
text_sensor:
  - platform: nartis_rf_meter
    name: "Meter Clock"   # Часы счётчика
    obis_code: "0.0.1.0.0.255"
    obis_class: 8
```

### Кнопка повторного сопряжения (button)

Кнопка в Home Assistant сбрасывает сохранённую сессию и запускает сопряжение заново - на случай, если связь со счётчиком «застряла». Запрос выполняется после завершения текущего цикла, поэтому активный обмен не прерывается.

```yaml
button:
  - platform: nartis_rf_meter
    name: "Re-pair"   # Повторное сопряжение
```

# Примеры конфигураций

Примечание: В названиях сенсоров рекомендуется использовать только буквы латинского алфавита.

### Трёхфазный счётчик - базовые показания

```yaml
external_components:
  - source: github://latonita/esphome-nartis-rf-meter
    components: [nartis_rf_meter]
    refresh: 1s

nartis_rf_meter:
  pin_sdio: GPIO23
  pin_sclk: GPIO18
  pin_csb: GPIO27
  pin_fcsb: GPIO26
  pin_gpio3: GPIO14
  meter_serial: "012345678901"
  #use_alternative_channels: true  # альтернативная сетка частот (если счётчик не отвечает)
  batch_size: 10                  # читать до 10 атрибутов одним запросом
  update_interval: 5min           # не засоряйте эфир, не делайте чаще

sensor:
  - platform: nartis_rf_meter
    name: "Voltage A"  # Напряжение фаза A
    obis_code: "1.0.32.7.0.255"
    unit_of_measurement: V
    accuracy_decimals: 1
    device_class: voltage
    state_class: measurement
    filters:
      - multiply: 0.1

  - platform: nartis_rf_meter
    name: "Voltage B" # Напряжение фаза B
    obis_code: "1.0.52.7.0.255"
    unit_of_measurement: V
    accuracy_decimals: 1
    device_class: voltage
    state_class: measurement
    filters:
      - multiply: 0.1

  - platform: nartis_rf_meter
    name: "Voltage C" # Напряжение фаза C
    obis_code: "1.0.72.7.0.255"
    unit_of_measurement: V
    accuracy_decimals: 1
    device_class: voltage
    state_class: measurement
    filters:
      - multiply: 0.1

  - platform: nartis_rf_meter
    name: "Current A" # Ток фаза A
    obis_code: "1.0.31.7.0.255"
    unit_of_measurement: A
    accuracy_decimals: 2
    device_class: current
    state_class: measurement
    filters:
      - multiply: 0.001

  - platform: nartis_rf_meter
    name: "Current B" # Ток фаза B
    obis_code: "1.0.51.7.0.255"
    unit_of_measurement: A
    accuracy_decimals: 2
    device_class: current
    state_class: measurement
    filters:
      - multiply: 0.001

  - platform: nartis_rf_meter
    name: "Current C" # Ток фаза C
    obis_code: "1.0.71.7.0.255"
    unit_of_measurement: A
    accuracy_decimals: 2
    device_class: current
    state_class: measurement
    filters:
      - multiply: 0.001

  - platform: nartis_rf_meter
    name: "Active power" # Активная мощность (сумма)
    obis_code: "1.0.1.7.0.255"
    unit_of_measurement: W
    accuracy_decimals: 1
    device_class: power
    state_class: measurement

  - platform: nartis_rf_meter
    name: "Energy total" # Энергия импорт
    obis_code: "1.0.1.8.0.255"
    unit_of_measurement: kWh
    accuracy_decimals: 0
    device_class: energy
    state_class: total_increasing

  - platform: nartis_rf_meter
    name: "Energy T1" # Энергия импорт - Тариф 1
    obis_code: "1.0.1.8.1.255"
    unit_of_measurement: kWh
    accuracy_decimals: 0
    device_class: energy
    state_class: total_increasing

  - platform: nartis_rf_meter
    name: "Energy T2" # Энергия импорт - Тариф 1
    obis_code: "1.0.1.8.2.255"
    unit_of_measurement: kWh
    accuracy_decimals: 0
    device_class: energy
    state_class: total_increasing

  - platform: nartis_rf_meter
    name: "Frequency" # Частота сети
    obis_code: "1.0.14.7.0.255"
    unit_of_measurement: Hz
    accuracy_decimals: 2
    device_class: frequency
    state_class: measurement
    filters:
      - multiply: 0.01

  - platform: nartis_rf_meter
    name: "Temperature"          # Температура
    obis_code: "0.0.96.9.0.255"
    unit_of_measurement: °C
    accuracy_decimals: 2
    device_class: temperature
    state_class: measurement
    filters:
      - multiply: 0.1

text_sensor:
  - platform: nartis_rf_meter
    name: "Meter Clock"          # Часы счётчика
    obis_code: "0.0.1.0.0.255"
    obis_class: 8
    icon: mdi:clock-outline
    entity_category: diagnostic

  - platform: nartis_rf_meter
    name: "Serial Number"        # Серийный номер
    obis_code: "0.0.96.1.0.255"

  - platform: nartis_rf_meter
    name: "Model"                # Модель
    obis_code: "0.0.96.1.1.255"

  - platform: nartis_rf_meter
    name: "Version"              # Версия
    obis_code: "0.0.96.1.2.255"

  - platform: nartis_rf_meter
    name: "Manufacturer"         # Производитель
    obis_code: "0.0.96.1.3.255"

  - platform: nartis_rf_meter
    name: "Firmware Version"     # Версия прошивки
    obis_code: "1.0.0.2.0.255"

  - platform: nartis_rf_meter
    name: "Current Tariff"       # Текущий тариф
    obis_code: "0.0.96.14.0.255"
    icon: mdi:counter

button:
  - platform: nartis_rf_meter
    name: "Re-pair"              # Повторное сопряжение

```

Полный пример - в файле [`example_esp32.yaml`](example_esp32.yaml).

# Диагностика и советы

- **Счётчик не отвечает / нет сопряжения.** Попробуйте `use_alternative_channels: true` либо зафиксируйте канал `fix_channel: 0...3`. Проверьте антенну и питание модуля.
- **Проверьте серийный номер.** `meter_serial` - ровно 12 цифр с шильдика; из него выводится стартовый ключ шифрования и адресация.
- **Логи.** Включите подробный вывод, чтобы видеть переходы состояний и дампы кадров:

  ```yaml
  logger:
    level: VERY_VERBOSE
    tx_buffer_size: 2048 # длинные кадры могут обрезаться на выводе без этой настройки
  ```

- **Повторное сопряжение.** Если связь со счётчиком теряется, после нескольких неудачных чтений подряд компонент сам делает повторное сопряжение на следующем интервале.

# Лицензия

См. файл [LICENSE](LICENSE).

# Справочник OBIS-кодов Нартис

Значения, подтверждённые на счётчиках Нартис И100/И300. В сенсоре указывайте
`obis_code` и при необходимости `obis_class` (по умолчанию 3 для `sensor`,
1 для `text_sensor`); атрибут значения - `obis_attr: 2`.

| Класс | OBIS-код | Описание |
|:-----:|----------|----------|
| 3 | `1.0.1.8.0.255` | Активная энергия, импорт (суммарно по тарифам) |
| 3 | `1.0.1.8.1.255` | Активная энергия, импорт - Тариф 1 |
| 3 | `1.0.1.8.2.255` | Активная энергия, импорт - Тариф 2 |
| 3 | `1.0.1.8.3.255` | Активная энергия, импорт - Тариф 3 |
| 3 | `1.0.1.8.4.255` | Активная энергия, импорт - Тариф 4 |
| 3 | `1.0.2.8.0.255` | Активная энергия, экспорт (суммарно по тарифам) |
| 3 | `1.0.2.8.1.255` | Активная энергия, экспорт - Тариф 1 |
| 3 | `1.0.2.8.2.255` | Активная энергия, экспорт - Тариф 2 |
| 3 | `1.0.2.8.3.255` | Активная энергия, экспорт - Тариф 3 |
| 3 | `1.0.2.8.4.255` | Активная энергия, экспорт - Тариф 4 |
| 3 | `1.0.3.8.0.255` | Реактивная энергия, импорт (суммарно по тарифам) |
| 3 | `1.0.3.8.1.255` | Реактивная энергия, импорт - Тариф 1 |
| 3 | `1.0.3.8.2.255` | Реактивная энергия, импорт - Тариф 2 |
| 3 | `1.0.3.8.3.255` | Реактивная энергия, импорт - Тариф 3 |
| 3 | `1.0.3.8.4.255` | Реактивная энергия, импорт - Тариф 4 |
| 3 | `1.0.4.8.0.255` | Реактивная энергия, экспорт (суммарно по тарифам) |
| 3 | `1.0.4.8.1.255` | Реактивная энергия, экспорт - Тариф 1 |
| 3 | `1.0.4.8.2.255` | Реактивная энергия, экспорт - Тариф 2 |
| 3 | `1.0.4.8.3.255` | Реактивная энергия, экспорт - Тариф 3 |
| 3 | `1.0.4.8.4.255` | Реактивная энергия, экспорт - Тариф 4 |
| 3 | `1.0.12.7.0.255` | Напряжение |
| 3 | `1.0.32.7.0.255` | Напряжение фазы А |
| 3 | `1.0.52.7.0.255` | Напряжение фазы В |
| 3 | `1.0.72.7.0.255` | Напряжение фазы С |
| 3 | `1.0.124.7.0.255` | Линейное напряжение AB |
| 3 | `1.0.125.7.0.255` | Линейное напряжение BC |
| 3 | `1.0.126.7.0.255` | Линейное напряжение CA |
| 3 | `1.0.91.7.0.255` | Ток нейтрали |
| 3 | `1.0.31.7.0.255` | Ток фазы А |
| 3 | `1.0.51.7.0.255` | Ток фазы В |
| 3 | `1.0.71.7.0.255` | Ток фазы С |
| 3 | `1.0.1.7.0.255` | Активная мощность (суммарно по фазам) |
| 3 | `1.0.21.7.0.255` | Активная мощность, фаза А |
| 3 | `1.0.41.7.0.255` | Активная мощность, фаза В |
| 3 | `1.0.61.7.0.255` | Активная мощность, фаза С |
| 3 | `1.0.3.7.0.255` | Реактивная мощность (суммарно по фазам) |
| 3 | `1.0.23.7.0.255` | Реактивная мощность, фаза А |
| 3 | `1.0.43.7.0.255` | Реактивная мощность, фаза В |
| 3 | `1.0.63.7.0.255` | Реактивная мощность, фаза С |
| 3 | `1.0.9.7.0.255` | Полная мощность (суммарно по фазам) |
| 3 | `1.0.29.7.0.255` | Полная мощность, фаза A |
| 3 | `1.0.49.7.0.255` | Полная мощность, фаза B |
| 3 | `1.0.69.7.0.255` | Полная мощность, фаза C |
| 3 | `1.0.13.7.0.255` | Коэффициент мощности (полный) |
| 3 | `1.0.33.7.0.255` | Коэффициент мощности, фаза A |
| 3 | `1.0.53.7.0.255` | Коэффициент мощности, фаза B |
| 3 | `1.0.73.7.0.255` | Коэффициент мощности, фаза C |
| 3 | `1.0.14.7.0.255` | Частота |
| 3 | `1.0.1.8.0.101` | Активная энергия, импорт (суммарно по тарифам) |
| 3 | `1.0.1.8.1.101` | Активная энергия, импорт - Тариф 1 |
| 3 | `1.0.1.8.2.101` | Активная энергия, импорт - Тариф 2 |
| 3 | `1.0.1.8.3.101` | Активная энергия, импорт - Тариф 3 |
| 3 | `1.0.1.8.4.101` | Активная энергия, импорт - Тариф 4 |
| 3 | `1.0.2.8.0.101` | Активная энергия, экспорт (суммарно по тарифам) |
| 3 | `1.0.2.8.1.101` | Активная энергия, экспорт - Тариф 1 |
| 3 | `1.0.2.8.2.101` | Активная энергия, экспорт - Тариф 2 |
| 3 | `1.0.2.8.3.101` | Активная энергия, экспорт - Тариф 3 |
| 3 | `1.0.2.8.4.101` | Активная энергия, экспорт - Тариф 4 |
| 3 | `1.0.3.8.0.101` | Реактивная энергия, импорт (суммарно по тарифам) |
| 3 | `1.0.3.8.1.101` | Реактивная энергия, импорт - Тариф 1 |
| 3 | `1.0.3.8.2.101` | Реактивная энергия, импорт - Тариф 2 |
| 3 | `1.0.3.8.3.101` | Реактивная энергия, импорт - Тариф 3 |
| 3 | `1.0.3.8.4.101` | Реактивная энергия, импорт - Тариф 4 |
| 3 | `1.0.4.8.0.101` | Реактивная энергия, экспорт (суммарно по тарифам) |
| 3 | `1.0.4.8.1.101` | Реактивная энергия, экспорт - Тариф 1 |
| 3 | `1.0.4.8.2.101` | Реактивная энергия, экспорт - Тариф 2 |
| 3 | `1.0.4.8.3.101` | Реактивная энергия, экспорт - Тариф 3 |
| 3 | `1.0.4.8.4.101` | Реактивная энергия, экспорт - Тариф 4 |
| 3 | `0.0.96.9.0.255` | Температура |
| 8 | `0.0.1.0.0.255` | Дата, время |
